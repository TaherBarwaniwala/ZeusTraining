...
class KeyboardEvents {
  ...
  onkeydown(e) {
    if (
      ...
      !this.grid.istyping &&
      ((e.keyCode > 36 && e.keyCode < 41) || e.code === "Enter")
    ) {
      if (e.code === "Enter") {
    ...
    if (this.grid.region.length > 0) {
      for (let i = 0; i < this.grid.region.length; i++) {
        if (this.grid.activecell === this.grid.region[i]) {
          ...
          this.grid.activecell =
            this.grid.region[(i + 1) % this.grid.region.length];
          this.grid.activecell.isFocus = true;
          break;
        }
      }
  ...
  handleArrawkeyDown(e) {
    try {
      ...
      if (
        this.grid.activecell &&
        this.grid.boundedrows.includes(
          this.grid.activecell.row.index.toString()
        ) &&
        this.grid.boundedcols.includes(
          this.grid.activecell.column.index.toString()
        )
      ) {
        this.grid.activecell.isFocus = true;
        this.grid.activecol = this.grid.activecell.column.index;
        this.grid.activerow = this.grid.activecell.row.index;
        if (this.grid.activecol)
          this.grid.columns[this.grid.activecol].header.isFocus = false;
        if (this.grid.activerow)
          this.grid.rows[this.grid.activerow].header.isFocus = false;
        if (e.keyCode === 37 || e.keyCode === 39) {
          ...
          this.grid.activecell.width =
            this.grid.columns[this.grid.activecol].cellWidth;
          ...
          this.grid.activecell.height =
            this.grid.rows[this.grid.activerow].cellHeight;
          this.grid.rows[this.grid.activerow].add_cell(this.grid.activecell);
          this.grid.columns[this.grid.activecol].add_cell(this.grid.activecell);
          this.grid.rows[this.grid.activerow].add_cell(this.grid.activecell);
        }
  ...
  handleCtrldown(e) {
    ...
    if (e.key === "f" || e.key === "F") {
      ...
      this.findAndReplace.createWindow("find");
    }
    if (e.key === "r" || e.key === "R") {
      this.grid.activecell = null;
      this.findAndReplace.createWindow("replace");
    }
  }
