...
class Grid {
  ...
  constructor(
    ...
    this.boundedcols = Column.getBoundedColumns(
      this.columns,
      this.topX + this.scrolloffsetX,
      this.width + this.scrolloffsetX
    );
    this.boundedrows = Row.getBoundedRows(
      this.rows,
      this.scrolloffsetY,
      this.height + this.scrolloffsetY
    );
    ...
    this.columnEvents = new ColumnEvents(
      this,
      this.columncanvas,
      this.Scrollbar
    );
    ...
    this.formsubmit = new FormSubmission(form);
    this.draw_triangle();
    this.onpointerdownallselectorbound = () => this.onpointerdownallselector();
    this.allselectorcanvas.addEventListener(
      "pointerdown",
      ...
      navigator.clipboard
        ...
        .finally(() => {
          ...
          if (this.region.length > 0) {
            ...
            for (
              let rowindex = 0;
              rowindex <= finalrow - initialrow - selectionrowrange + 1;
              rowindex += selectionrowrange
            ) {
              for (
                let colindex = 0;
                colindex <= finalcol - initialcol - selectioncolrange + 1;
                colindex += selectioncolrange
              ) {
                if (!this.rows.hasOwnProperty(initialrow + rowindex)) {
                  let row = new Row(
                    initialrow + rowindex,
                    this.x,
                    this.rows[initialrow + rowindex - 1].y +
                      this.rows[initialrow + rowindex - 1].cellHeight,
                    this.cellWidth,
                    this.cellHeight,
                    this.canvas,
                    this.rowcanvas
                ...
                if (!this.columns.hasOwnProperty(initialcol + colindex)) {
                  let col = new Column(
                    initialcol + colindex,
                    this.columns[initialcol + colindex - 1].x +
                      this.columns[initialcol + colindex - 1].cellWidth,
                    this.y,
                    this.cellWidth,
                    this.cellHeight,
                    this.canvas,
                ...
                cell = this.rows[initialrow + rowindex].getCell(
                  initialcol + colindex
                )
                  ? this.rows[initialrow + rowindex].getCell(
                      initialcol + colindex
                    )
                  : Cell.createCell(
                      this.rows[initialrow + rowindex],
                      this.columns[initialcol + colindex]
                    );
                activecell.push(cell);
              }
            }
            if (
              !activecell.includes(this.rows[initialrow].getCell(initialcol))
            ) {
  ...
  paste_selection(activecell, selection) {
    ...
    for (let rowoffset = 0; rowoffset < rowcount; rowoffset++) {
      if (!this.rows.hasOwnProperty(initialrow + rowoffset)) {
        let row = new Row(
          initialrow + rowoffset,
          this.x,
          this.rows[initialrow + rowoffset - 1].y +
            this.rows[initialrow + rowoffset - 1].cellHeight,
          this.cellWidth,
          this.cellHeight,
          this.canvas,
          this.rowcanvas
      ...
      for (let columnoffset = 0; columnoffset < colcount; columnoffset++) {
        if (!this.columns.hasOwnProperty(initialcol + columnoffset)) {
          let col = new Column(
            initialcol + columnoffset,
            this.columns[initialcol + columnoffset - 1].x +
              this.columns[initialcol + columnoffset - 1].cellWidth,
            this.y,
            this.cellWidth,
            this.cellHeight,
            this.canvas,
        ...
        cell = this.rows[initialrow + rowoffset].getCell(
          initialcol + columnoffset
        )
          ? this.rows[initialrow + rowoffset].getCell(initialcol + columnoffset)
          : Cell.createCell(
              this.rows[initialrow + rowoffset],
              this.columns[initialcol + columnoffset]
            );
        cell.text = selection[rowoffset][columnoffset]
          ? selection[rowoffset][columnoffset]
          : "";
      ...
      for (let row = initialrow; row <= finalrow; row++) {
        for (let col = initialcol; col <= finalcol; col++) {
          if (
            ...
            this.columns[col] &&
            this.rows[row].cells &&
            this.rows[row].cells.hasOwnProperty(col)
          )
  ...
  draw_copy_region() {
    if (this.copyregion.length > 0) {
      ...
      let bottomx =
        this.copyregion[this.copyregion.length - 1].x +
        this.copyregion[this.copyregion.length - 1].width;
      let bottomy =
        this.copyregion[this.copyregion.length - 1].y +
        this.copyregion[this.copyregion.length - 1].height;
      this.copyregion.forEach((cell) => {
        topx = topx < cell.x ? topx : cell.x;
        topy = topy < cell.y ? topy : cell.y;
        bottomx = bottomx > cell.x + cell.width ? bottomx : cell.x + cell.width;
        bottomy =
          bottomy > cell.y + cell.height ? bottomy : cell.y + cell.height;
      });
      topx -= scrolloffsetX;
      ...
      if (this.columnselection.length > 0 || this.rowselection.length > 0) {
        this.ctx.strokeRect(
          (topx < bottomx ? topx : bottomx) + 2,
          (topy < bottomy ? topy : bottomy) + 2,
          Math.abs(topx - bottomx) - 3,
          Math.abs(topy - bottomy) - 3
        );
      } else {
        this.ctx.strokeRect(
          (topx < bottomx ? topx : bottomx) + 2,
  ...
  draw() {
    ...
    if (this.columnselection.length !== 0)
      this.columnEvents.draw_selectedcols_background();
    if (this.rowselection.length !== 0)
      this.rowEvents.draw_selectedrows_background();
    for (const row of this.boundedrows) {
    ...
    if (this.rowselection.length === 0)
      this.columnEvents.draw_selectedcols_border();
    if (this.columnselection.length === 0)
      this.rowEvents.draw_selectedrows_border();
    if (this.activecell)
      this.activecell.draw(this.scrolloffsetX, this.scrolloffsetY);
    this.draw_copy_region();
  ...
  async getMinMaxAvgCount() {
    ...
    if (numcount > 0) {
      ...
      this.footerctx.fillRect(
        0,
        0,
        this.width + this.topX,
        this.height + this.topY
      );
  ...
  set_bounding_region() {
    ...
    if (this.boundedcols.length < 1) {
      ...
      for (let col of columnkeys) {
        if (
          Math.abs(
            this.columns[col].x +
              this.columns[col].cellWidth -
              this.scrolloffsetX
          ) <
            Math.abs(
              this.columns[closestCol].x +
                this.columns[closestCol].cellWidth -
                this.scrolloffsetX
            ) ||
          Math.abs(this.columns[col].x - this.scrolloffsetX) <
            Math.abs(this.columns[closestCol].x - this.scrolloffsetX)
        ) {
          closestCol = col;
        }
      }
      if (this.columns[closestCol].x - this.scrolloffsetX > 0) {
        ...
        index -= Math.floor(
          (this.columns[closestCol].x - this.scrolloffsetX) / this.cellWidth
        );
        let x = this.columns[closestCol].x - index * this.cellWidth;
        this.columns[index.toString()] = new Column(
          index,
          x,
          15,
          this.cellWidth,
    ...
    if (this.boundedrows.length < 1) {
      ...
      for (let row of rowkeys) {
        if (
          Math.abs(
            this.rows[row].y + this.rows[row].cellHeight - this.scrolloffsetY
          ) <
            Math.abs(
              this.rows[closestRow].y +
                this.rows[closestRow].cellHeight -
                this.scrolloffsetY
            ) ||
          Math.abs(this.rows[row].y - this.scrolloffsetY) <
            Math.abs(this.rows[closestRow].y - this.scrolloffsetY)
        ) {
          closestRow = row;
        }
      }
      if (this.rows[closestRow].y - this.scrolloffsetY > 0) {
        ...
        index -= Math.floor(
          (this.rows[closestRow].y - this.scrolloffsetY) / this.cellHeight
        );
        let y = this.rows[closestRow].y - index * this.cellHeight;
        this.rows[index.toString()] = new Row(
          index,
          0,
          y,
          this.cellWidth,
    ...
    while (
      this.width -
        (this.columns[this.boundedcols[this.boundedcols.length - 1]].x +
          this.columns[this.boundedcols[this.boundedcols.length - 1]]
            .cellWidth -
          this.scrolloffsetX) >
      0
    ) {
      ...
      if (this.columns.hasOwnProperty(index + 1)) {
        ...
        let newcol = new Column(
          index + 1,
          this.columns[index].x + this.columns[index].cellWidth,
          15,
          this.cellWidth,
          this.cellHeight,
          this.canvas,
    ...
    while (
      this.columns[this.boundedcols[0]].x - this.x - this.scrolloffsetX >
      0
    ) {
    ...
    while (
      this.height -
        (this.rows[this.boundedrows[this.boundedrows.length - 1]].y +
          this.rows[this.boundedrows[this.boundedrows.length - 1]].cellHeight -
          this.scrolloffsetY) >
      0
    ) {
      ...
      if (this.rows.hasOwnProperty(index + 1)) {
        ...
        let newrow = new Row(
          index + 1,
          0,
          this.rows[index].y + this.rows[index].cellHeight,
          this.cellWidth,
          this.cellHeight,
          this.canvas,
          this.rowcanvas
    ...
    let prevEnd =
        this.rows[this.boundedrows[0]].y +
        this.rows[this.boundedrows[0]].cellHeight,
      nextStart;
    for (let row = 1; row < this.boundedrows.length; row++) {
      ...
      if (nextStart - prevEnd > 0) {
        while (
          nextStart -
            (this.rows[this.boundedrows[row - 1]].y +
              this.rows[this.boundedrows[row - 1]].cellHeight) >
          0
        ) {
    ...
    (prevEnd =
      this.columns[this.boundedcols[0]].x +
      this.columns[this.boundedcols[0]].cellWidth),
      nextStart;
    for (let col = 1; col < this.boundedcols.length; col++) {
      ...
      if (nextStart - prevEnd > 0) {
        while (
          nextStart -
            (this.columns[this.boundedcols[col - 1]].x +
              this.columns[this.boundedcols[col - 1]].cellWidth) >
          0
        ) {
      ...
      for (let j = 0; j < responseArray.length; j++) {
        ...
        if (rowobj === null || rowobj === undefined) {
          ...
          rowobj = new Row(
            row + j,
            0,
            this.rows[row + j - 1].y + this.rows[row + j - 1].cellHeight,
            this.cellWidth,
            this.cellHeight,
            this.canvas,
            this.rowcanvas
        ...
        i = 1;
        for (let key in res) {
          if (res[key] && res[key] !== null && key !== "rownum") {
