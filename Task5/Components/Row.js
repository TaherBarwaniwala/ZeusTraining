...
class Row {
  ...
  fill(y) {
    this.ctx.save();
    this.ctx.fillStyle = this.isSelected
      ? this.selectFillStyle
      : this.fillStyle;
  ...
  edgehittest(y) {
    return (
      y <= this.y + this.cellHeight + 2 && y >= this.y + this.cellHeight - 2
    );
  }
  /**
   *
  ...
  moveShadow(y) {
    ...
    this.ctx.restore();
    this.headerctx.save();
    this.headerctx.fillStyle = this.isSelected
      ? this.selectFillStyle
      : this.fillStyle;
  ...
  static create_shadowrow(rows) {
    ...
    return new Row(
      ...
      initialrow.x,
      initialrow.y,
      initialrow.cellWidth,
      cellHeight,
      initialrow.canvas,
      initialrow.headercanvas,
      "0.1",
      "#000000",
      "rgba(202, 234, 216,0.5)"
    );
  }
  /**
   *
  ...
  static getBoundedRows(rows, y, h) {
    ...
    for (let row in rows) {
      if (
        (rows[row].y >= y && rows[row].y + rows[row].cellHeight <= y + h) ||
        (rows[row].y <= y && rows[row].y + rows[row].cellHeight >= y + h) ||
        (rows[row].y >= y && rows[row].y <= y + h) ||
        (rows[row].y + rows[row].cellHeight >= y &&
          rows[row].y + rows[row].cellHeight <= y + h)
      ) {
  ...
  static removeRows(rows, boundedrows) {
    for (let row in rows) {
      if (
        boundedrows.indexOf(row) < 0 &&
        Object.keys(rows[row].cells).length === 0 &&
        !rows[row].header.isFocus &&
        !rows[row].header.isSelected &&
        rows[row].initialHeight === rows[row].cellHeight
      ) {
        delete rows[row];
      }
    }
  ...
  static CreateRow(rowIndex, rows, cellWidth, cellHeight) {
    ...
    if (nearestIndex < rowIndex) {
      return new Row(
        ...
        0,
        rows[nearestIndex].y +
          rows[nearestIndex].cellHeight +
          (rowIndex - nearestIndex - 1) * cellHeight,
        cellWidth,
        cellHeight,
      ...
      return new Row(
        rowIndex,
        0,
        rows[nearestIndex].y + (nearestIndex - rowIndex + 1) * cellHeight,
        cellWidth,
        cellHeight,
        rows[nearestIndex].canvas,
        rows[nearestIndex].headercanvas
